...
function ContractTemplates({ query, onMessage }: { query: AsyncQuery<Awaited<ReturnType<typeof getContractTemplates>>>; onMessage: (value: string) => void }) {
  ...
  async function save(event: FormEvent) { event.preventDefault(); setBusy(true); try { await saveContractTemplate({ id: form.id || undefined, name: form.name, kind: form.kind, terms: form.terms || null, active: form.active }); setForm(blank); setOpen(false); await query.refresh(); onMessage("Contract template saved."); } catch (error) { onMessage(error instanceof Error ? error.message : "Unable to save template."); } finally { setBusy(false); } }
  return <>
    ...
    {open && <form className="admin-form" onSubmit={save}><h2>{form.id ? "Edit" : "New"} contract template</h2><div className="form-grid"><label>Template name<input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label>Type<select value={form.kind} onChange={(event) => setForm({ ...form, kind: event.target.value as "setup" | "teardown" })}><option value="setup">Setup</option><option value="teardown">Teardown</option></select></label></div><label className="terms-editor">Published terms<textarea required value={form.terms} onChange={(event) => setForm({ ...form, terms: event.target.value })} /></label><label className="checkbox-field"><input type="checkbox" checked={form.active} onChange={(event) => setForm({ ...form, active: event.target.checked })} /> Active template</label><button className="button primary" disabled={busy}><Save /> Save template</button></form>}
